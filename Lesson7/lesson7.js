...
var block_timer; // Таймер препятствий
...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && new_unit !== undefined && snakeInBlock(new_unit)) {
...
function snakeInBlock(newUnit) {
    var check = false;

    var newUnitClasses = newUnit.getAttribute('class').split(' ');
    if (!newUnitClasses.includes('block-unit')) {
        check = true;
    }
    return check;
}
